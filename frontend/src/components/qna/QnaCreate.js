...
function QnaCreate() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError('');
    if (!title.trim()) return setError('Please enter a title for your Q&A.');

    setSubmitting(true);
    try {
      const ownerToken = getVoterToken();
      const res = await axios.post(`${API}/qna`, { title: title.trim(), ownerToken });
      // Remember that this browser owns the board (can mark answers).
      localStorage.setItem(`quorum_qna_owner_${res.data.id}`, '1');
      navigate(`/features/qna/${res.data.id}`);
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to create board. Is the backend running?');
      setSubmitting(false);
    }
  };
    ...
    <div className="mx-auto max-w-2xl px-4 py-14 sm:px-6">
      <div className="text-center">
        ...
        <p className="mt-2 text-slate-600">
          Let your audience ask questions and upvote the ones they care about most.
        </p>
      </div>
      ...
      >
        <label className="block text-sm font-semibold text-slateink">Q&amp;A title</label>
        <input
          type="text"
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          placeholder="e.g. Ask me anything — Team Meeting"
          maxLength={255}
